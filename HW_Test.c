...
int main()
{
  ...
  printf("\n");
  if (mousePresent)
    printf("Looking for Mouse Scroll Wheel and Buttons\n");
  if (touchPresent)
    printf("Looking for Touches \n");
...
void processMouse(int mbut)
{
  if (mbut == 128) // scroll whell turned
  {
    ...
    if (mouseScroll > 0)
      printf("Scroll Wheel Clicked Clockwise\n");
    if (mouseScroll < 0)
      printf("Scroll Wheel Clicked Anticlockwise\n");
    mouseScroll = 0;
  }
