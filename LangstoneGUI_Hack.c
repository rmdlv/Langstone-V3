...
void setHwRxFreq(double fr)
{
  ...
  if (rxfreqhz < 1000000)
    rxfreqhz = 100000; // this is the lowest frequency we can receive
...
void changeSetting(void)
{
  ...
  if (settingNo == TX_GAIN) // Tx Attenuator
  {
    ...
    mouseScroll = 0;
    if (bandTxGain[band] > 47)
      bandTxGain[band] = 47;
    if (bandTxGain[band] < 0)
      bandTxGain[band] = 0;
    setHackTxGain(bandTxGain[band]);
    displaySetting(settingNo);
  }
  ...
  if (settingNo == RX_GAIN) // Rx Gain Setting
  {
    ...
    mouseScroll = 0;
    if (bandRxGain[band] < 0)
      bandRxGain[band] = 0;
    if (bandRxGain[band] > 40)
      bandRxGain[band] = 40;
    setHackRxGain(bandRxGain[band]);
    displaySetting(settingNo);
  }
  ...
  if (settingNo == RX_BASE) // Rx Baseband Setting
  {
    ...
    mouseScroll = 0;
    if (bandRxBase[band] < 0)
      bandRxBase[band] = 0;
    if (bandRxBase[band] > 62)
      bandRxBase[band] = 62;
    setHackRxBase(bandRxBase[band]);
    displaySetting(settingNo);
  }
